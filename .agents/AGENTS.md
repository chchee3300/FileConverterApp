# Gemini AI Assistant Rules (FileConverterApp)

## 核心規則：強制大範圍全類別測試 (Automatic Wide-Range Testing)
在每次修改程式碼（尤其是 `main.js` 或 `index.html` 的核心邏輯）之後，Gemini 必須確保「所有檔案類別」的轉換邏輯沒有因為程式碼的變動而被意外破壞（Regression）。
...
### Gemini 執行動作：
- **靜態分析**：在完成程式碼修改後，Gemini 必須在輸出的思考過程中，主動追蹤上述 4 大類別的 `command` 生成邏輯是否依舊正確。
